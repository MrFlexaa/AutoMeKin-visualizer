...
def bokeh_network_view(G,positions=None,width=800,height=600,graph_title="Reaction network representation",out_backend="canvas"):
    ...
    bfig.xgrid.grid_line_color = 'white'
...
def profile_bokeh_plot(G,profile_list,condition=[],width=600,height=600,out_backend="canvas"):
    ...
    for ii,cdspath in enumerate(cds_paths):
        ...
        rx_label = bkm.LabelSet(x="x",y="y",text="lab",source=cds_lab_textlab,x_offset=style_information["profileLabelXOffset"],y_offset=style_information["profileLabelYOffset"],
                                name="THELABELS",text_font_size=style_information["profileLabelFont"])  ####,text_font='Latex'
        ...
        energy_label = bkm.LabelSet(x="x",y="y",text="elab",source=cds_lab_elab,x_offset=style_information["profileLabelXOffset"],
                                    ...
                                    text_font_size=style_information['profileLabelFont']) #####,text_font='Latex'
...
def show_energy(prof):
	...
	# show or hide for ODD ii values (energy label) AND only for visible renderers
	Nlines = int(len(prof.renderers)/2)
	for i in range(0,Nlines):
		jj = 2*i + 1
		if (prof.renderers[jj-1].visible):
			prof.center[jj+2].visible = True
	return None

def select_by_mol(prof,sel_spc):
	# Profile selection by molecule name
	Nlines = int(len(prof.renderers)/2)
	for ii in range(0,Nlines):
		# access only even entries
		jj = 2*ii
		current_data = prof.renderers[jj].data_source
		labels = current_data.data["lab"]
		# Hide all profiles whose labels do not contain sel_spc 
		if (sel_spc not in labels):
			prof.renderers[jj].visible = False
			prof.renderers[jj+1].visible = False
			# and for tag/energy labels
			prof.center[jj+2].visible = False
			prof.center[jj+3].visible = False
	return None

def hide_by_mol(prof,unsel_spc):
	# Exclude a profile which contains a given molecule
	Nlines = int(len(prof.renderers)/2)
	for ii in range(0,Nlines):
		# access only even entries
		jj = 2*ii
		current_data = prof.renderers[jj].data_source
		labels = current_data.data["lab"]
		# Hide all profiles whose labels DO contain unsel_spc 
		if (unsel_spc in labels):
			prof.renderers[jj].visible = False
			prof.renderers[jj+1].visible = False
			# and for tag/energy labels
			prof.center[jj+2].visible = False
...
def reset_profiles(prof,show_e=True):
	# Make all profiles visible, like the reset button in the GUI
	Nlines = int(len(prof.renderers)/2)
	for ii in range(0,Nlines):
		jj = 2*ii
		prof.renderers[jj].visible = True
		prof.renderers[jj+1].visible = True
		# and for tag/energy labels
		prof.center[jj+2].visible = True
		prof.center[jj+3].visible = show_e
	return None

def rescale_profiles(prof,rescale_factor=1.0):
	# Get larger or smaller profiles for the image
	Nlines = int(len(prof.renderers)/2)
	for ii in range(0,Nlines):
		# access only even entries
		jj = 2*ii
		# modify all x values for renderers & labels: be careful with casting
		prof.renderers[jj].data_source.data["x"] *= rescale_factor
		midpoints = np.array([(float(item)+0.5*rescale_factor)
							  for item in prof.renderers[jj].data_source.data["x"][::2]])
		prof.renderers[jj+1].data_source.data["x"] = midpoints
		prof.renderers[jj+1].glyph.width = rescale_factor
		# labels & energies
		prof.center[jj+2].source.data["x"] *= rescale_factor
		prof.center[jj+3].source.data["x"] *= rescale_factor
	return None

def label_shift(profile,target_label,label_shift):
	# Shift a specific label in a profile
	Nlines = int(len(profile.renderers)/2)
	for ii in range(0,Nlines):
		# access only even entries
		jj = 2*ii
		current_data = profile.renderers[jj+1].data_source
		if (target_label in current_data.data["lab"]):
			loc = [ndx for ndx,lab in enumerate(current_data.data["lab"]) if lab == target_label][0]
			profile.center[jj+2].source.data["y"][loc] += label_shift
	return None
...
def label_xshift(data,target_label, label_shift):
    if target_label in data['lab']:
        loc = [ndx for ndx,lab in enumerate(data["lab"]) if lab == target_label][0]
        if len(data['x'])<2:
            data['x'][1]=2.5

def recolor_profile(profile,new_palette):
	'''Go through the renderers and modify the glyph colors: as we have lines and rectangles in pairs,
	we need to consider line color for the 1st and fill color for the 2nd'''
	Nitems = len(profile.renderers)
	for jj in range(0,Nitems,2):
		r1 = profile.renderers[jj]
		r2 = profile.renderers[jj+1]
		ii = int(jj/2)

		# cycle the palette -> return to start if there are not enough colors
		if (ii >= len(new_palette)):
			ii = 0
		r1.glyph.line_color = new_palette[ii].lower()
		r2.glyph.fill_color = new_palette[ii].lower()
		# also make it larger: orig. dimensions are lw=4 and height=2.5
		r1.glyph.line_width = 4
		r2.glyph.height = 5
	return None

def read_and_path(finaldir,rxnfile,barrierless,source,target,ethr=None):
	# Graph input function
	data = arx.RX_parser(finaldir,rxnfile)
	if (barrierless):
		data_barrless = arx.RX_parser(finaldir,"RXNet.barrless")
		data[:] = [data[ii] + data_barrless[ii] for ii in range(len(data))]
	G = arx.RX_builder(finaldir,data)

	# prepare paths
	limits = arx.node_synonym_search(G,[source,target])
	path_list = arx.add_paths(G,limits[0],limits[1],cutoff=4,skip_int_frags=False)
	if (ethr):
		path_list[:] = arx.path_filter(G,path_list,ethr)
